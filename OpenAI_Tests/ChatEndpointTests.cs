	...
	public class ChatEndpointTests
	{
		...
		public async Task SummarizeFunctionResult()
		{
			try
			{
                ...
                conversation.AppendUserInput("What is the weather like in Baltimore?");

                var result = await conversation.GetResultFromChatbotAsync();
				...
				if (result != null)
				{
					...
					else if (result.NextAction == OpenAI_API.Chat.Action.EOS)
					{
